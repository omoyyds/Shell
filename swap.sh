...
# 计算 swap 大小范围
min_swap_size=$((mem_size * 1))  # 最小值设为物理内存的1倍
max_swap_size=$((mem_size * 2))  # 最大值设为物理内存的2倍
...
echo "正在创建大小为物理内存两倍 (${max_swap_size}M) 的 swapfile..."
...
if grep -q "swapfile" /etc/fstab && swapon -s | grep -q "/swapfile"; then
  echo "虚拟内存已成功设置为物理内存的两倍 (${max_swap_size}M)。"
else
